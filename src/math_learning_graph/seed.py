...
def _merge_knowledge_points() -> list[KnowledgePoint]:
    core_points = [
        ...
        KnowledgePoint(
            ...
            conceptual_layers=[
                "两个未知数、都是一次，就是二元一次方程：x+y=10 说的是「两个数凑成 10」。",
                "一个方程管不死两个未知数：x+y=10 的解有无穷多组——"
                "一个条件只能「限制」，不能「锁死」。",
                "把每组解 (x, y) 画进坐标系，正好排成一条直线：方程和图像从这里开始牵手。",
            ],
            worked_examples=[
                _worked(
                    "鸡兔同笼的第一步",
                    "鸡和兔共 10 只。设鸡 x 只、兔 y 只，能列出什么方程？能解出来吗？",
                    [
                        "「共 10 只」翻译成 x+y=10。",
                        "试解：x=1,y=9 行；x=2,y=8 也行……无穷多组都满足。",
                        "光知道总只数定不下来——还差一个条件（比如共 28 条腿）。"
                        "一个方程是一条约束，两个未知数需要两条。",
                    ],
                    "列出 x+y=10 的五组解，观察 x 增大时 y 怎么变。",
                ),
            ],
            practice_ladder=[
                _task(
                    "看懂",
                    "x+y=7，随口说出三组解，再说说为什么说不完。",
                    "体会解集无穷。",
                ),
                _task(
                    "会做",
                    "把 2x+y=8 改写成 y=8−2x，算出 x=0、1、2、3 时的 y。",
                    "用一个量表达另一个量。",
                ),
                _task(
                    "迁移",
                    "把上面四组 (x, y) 描到坐标系里，看看它们排成了什么。",
                    "方程变直线的第一眼。",
                ),
            ],
            reflection_questions=[
